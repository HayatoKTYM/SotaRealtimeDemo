...
def getRep(bgrImg):
    ...
    alignedFaces = []
    for box in bb:
        face = align.align(96,rgbImg,box,
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            plt.ylim([0,1.2])
            plt.bar(["Look"],[confidences[0]])
            plt.bar(["Not Look"],[confidences[1]])
            plt.savefig('predict.png')
            fff = cv2.imread('predict.png')
            cv2.imshow("predict",fff)
            ggg = cv2.imread('VAD_A.png')
            cv2.imshow("VAD_A",ggg)
        except:
            pass
        k = {"non-looking":0,"looking":1}
        for idx, person in enumerate(notations):
            cv2.rectangle(frame2, (bbs[idx].left(), bbs[idx].top()),
